...
export default function PropertyDetails() {
  ...
  const toggleFav = () => {
    const next = favorites.includes(property.id) ? favorites.filter(x => x !== property.id) : [...favorites, property.id];
    setFavorites(next);
  };
  const toggleCompare = () => {
    const next = compare.includes(property.id) ? compare.filter(x => x !== property.id) : [...compare, property.id];
    setCompare(next);
  };
      ...
      <div className="details-grid">
        <div>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <div className="property-price" style={{ marginTop: 6 }}>₹{property.price.toLocaleString()}</div>
            <div className="property-meta" style={{ marginTop: 4 }}>{property.bedrooms} beds • {property.sqft} sqft • {property.city}</div>
            ...
            <section style={{ marginTop: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <div>
                <h4>Nearby</h4>
                <div className="map-placeholder">Map & POI placeholder</div>
              </div>
            </section>

            <section style={{ marginTop: 14 }}>
              <h4>Floor Plan</h4>
              <div className="map-placeholder">Interactive floor-plan placeholder</div>
            </section>
          </div>
        </div>
        ...
        <aside>
          <div className="agent-box">
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
              <button className="btn-outline" onClick={toggleFav}>{favorites.includes(property.id) ? "Remove Favorite" : "Add to Favorites"}</button>
              <button className="btn-outline" onClick={toggleCompare}>{compare.includes(property.id) ? "Remove" : "Compare"}</button>
            </div>
